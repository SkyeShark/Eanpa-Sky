...
export function makeRingCloudField(T, uniforms, time, {width=2048,height=64,refreshSeconds=.2}={}) {
    ...
    const coordinate=T.uv(),drift=uniforms.wind.mul(time);
    const octave=(n,weight,direction,warp)=>{
        let p=coordinate.add(drift.mul(direction)).mul(vec2(n,n/33.4));
    ...
    const coverage=smoothstep(threshold,threshold.add(.15),field).mul(edge)
    ...
    return {stats,target,
        sample(uv){const data=T.texture(target.texture,uv);return vec2(data.r.mul(ready),data.g);},
        async prepare(renderer,t,force=false){
            ...
            try {
                ...
                await quad.renderAsync(renderer);ready.value=1;last=t;stats.lastTime=t;stats.captures++;
