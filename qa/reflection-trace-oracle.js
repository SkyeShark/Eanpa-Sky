(async()=>{
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){const i=(y*w+x)*4;data[i]=(x+.5)/w;data[i+1]=(y+.5)/h;data[i+3]=1;}
    const coordinates=new T.DataTexture(Uint16Array.from(data,T.DataUtils.toHalfFloat),w,h,T.RGBAFormat,T.HalfFloatType);
    coordinates.generateMipmaps=true;coordinates.minFilter=T.LinearMipmapLinearFilter;coordinates.magFilter=T.LinearFilter;coordinates.needsUpdate=true;
    const depth=T.texture(p.history.depthTexture,T.screenUV),ids=T.texture(p.history.textures[1],T.screenUV);
    const normal=T.texture(p.history.textures[1],T.screenUV),rough=T.texture(p.scenePass.getTexture('metalrough'),T.screenUV).g;
    const projection=T.uniform(_c.projectionMatrix),inverse=T.uniform(_c.projectionMatrixInverse);
    const tracer=makeScreenSpaceTrace({colorNode:T.texture(coordinates,T.screenUV),depthNode:depth,
        objectIdNode:T.sample(uv=>ids.load(uv.mul(T.textureSize(ids)).floor()).a),camera:_c,projection,
        projectionInverse:inverse,near:T.uniform(_c.near),far:T.uniform(_c.far),...p.ssrNode});
    ...
    try{
        ...
        const [hit,keys]=(await Promise.all([r.readRenderTargetPixelsAsync(target,0,0,w,h),r.readRenderTargetPixelsAsync(p.history,0,0,w,h,1)])).map(unpack);
        const f=T.DataUtils.fromHalfFloat,rows=[];let accepted=0,self=0;
        for(let y=0;y<h;y++)for(let x=0;x<w;x++){
            const i=(y*w+x)*4;if(f(keys[i+3])!==2||hit[i+3]<.1)continue;accepted++;
            const hx=Math.min(w-1,Math.max(0,Math.floor(hit[i]/hit[i+3]*w))),hy=Math.min(h-1,Math.max(0,Math.floor(hit[i+1]/hit[i+3]*h)));
            const key=f(keys[(hy*w+hx)*4+3]);if(key===2)self++;
            if(rows.length<12&&accepted%139===0)rows.push({pixel:[x,y],hit:[hx,hy],key,confidence:hit[i+3]});
        }
        return{accepted,self,rows};
    }finally{T.RendererUtils.restoreRendererState(r,saved);target.dispose();mat.dispose();coordinates.dispose();}
})()
