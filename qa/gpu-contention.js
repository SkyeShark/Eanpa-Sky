...
(async()=>{
    ...
    const input=device.createBuffer({size:16*1024*1024,usage:GPUBufferUsage.STORAGE,mappedAtCreation:true});
    ...
    const output=device.createBuffer({size:1024*256*4,usage:GPUBufferUsage.STORAGE});
    ...
    for(let attempt=0;attempt<5;attempt++){
        ...
        iterations=Math.max(1,Math.min(2048,Math.round(iterations*targetMs/Math.max(ms,.01))));
    }
    device.queue.writeBuffer(params,0,new Uint32Array([iterations,0,0,0]));
    const owner=_reflectionPipeline,original=owner.render;
    let disposed=false;
    const stop=()=>{if(disposed)return;disposed=true;owner.render=original;clearTimeout(watchdog);
        for(const buffer of[input,output,params,resolve,read])buffer.destroy();query.destroy();delete globalThis.__gpuContention;};
    const watchdog=setTimeout(stop,180000);
    owner.render=async function(...args){const result=await original.apply(this,args);dispatch();await device.queue.onSubmittedWorkDone();return result;};
