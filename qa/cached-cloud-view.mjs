...
try{
 const result=await c.evaluate(`(async()=>{
  ...
  const start=f.time+4;
  for(let i=0;i<80;i++){await new Promise(requestAnimationFrame);await f.frame(start+i/60);}
 ...
 await mkdir('qa/review/cached-performance',{recursive:true});
 await writeFile(`qa/review/cached-performance/${name}.jpg`,Buffer.from(shot.data,'base64'));
 await writeFile(`qa/review/cached-performance/${name}.json`,JSON.stringify(result,null,2));
