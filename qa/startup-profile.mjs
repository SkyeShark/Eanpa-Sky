...
let last='';new MutationObserver(()=>{const boot=document.getElementById('boot');if(!boot)return;const text=boot.style.display==='none'?'ready':boot.textContent;if(text!==last){last=text;p.stages.push({text,at:performance.now()});if(text==='ready'){p.readyAt=performance.now();p.done=true;if(${pauseAtReady}&&globalThis._eanpaTest)_eanpaTest.paused=true;}}}).observe(document,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['style']});
