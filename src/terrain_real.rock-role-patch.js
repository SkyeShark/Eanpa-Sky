...
const DESERT_ROCK_EMBEDDED_NATURAL_TARGET = 32;
const DESERT_ROCK_EMBEDDED_CLIFF_TARGET = 32;
...
const DESERT_ROCK_NATURAL_EMBEDDED_MIN_GRADE = 0.45;
const DESERT_ROCK_CLIFF_EMBEDDED_MIN_GRADE = 0.35;
const DESERT_ROCK_CLIFF_SHOULDER_MASK_RANGE = Object.freeze([0.08, 0.75]);
const DESERT_ROCK_EMBEDDED_BURY_FRACTION_RANGE = Object.freeze([0.40, 0.65]);
const DESERT_ROCK_DERIVATIVE_WINDING_REPAIRS = 31;
const DESERT_ROCK_PIECE_GROUPS = Object.freeze({
    largeGroundSlab: Object.freeze([1, 2, 4, 8, 9]),
    mediumGroundIrregular: Object.freeze([3, 5]),
    smallRounded: Object.freeze([0, 6, 7, 10]),
    steepEmbedded: Object.freeze([11]),
});
const DESERT_ROCK_PIECE_CLASS = Object.freeze([
    'smallRounded', 'largeGroundSlab', 'largeGroundSlab', 'mediumGroundIrregular',
    'largeGroundSlab', 'mediumGroundIrregular', 'smallRounded', 'smallRounded',
    'largeGroundSlab', 'largeGroundSlab', 'smallRounded', 'steepEmbedded',
]);
const DESERT_ROCK_DIAMETER_RANGES = Object.freeze({
    largeGroundSlab: Object.freeze([1.8, 5.5]),
    mediumGroundIrregular: Object.freeze([0.9, 2.8]),
    smallRounded: Object.freeze([0.35, 1.15]),
    // Piece 11 is sized along its authored local-Y long axis, not its footprint.
    steepEmbedded: Object.freeze([1.4, 3.4]),
});
...
function makeEmbeddedRockPlacement({
    x,
    z,
    grade,
    nearField,
    ...
    const range = DESERT_ROCK_DIAMETER_RANGES.steepEmbedded;
    const sizeT = sizeSeed ** 1.2;
    const desiredLongAxis = range[0] + (range[1] - range[0]) * sizeT;
    const buryRange = DESERT_ROCK_EMBEDDED_BURY_FRACTION_RANGE;
    return {
        x,
        z,
        piece: DESERT_ROCK_PIECE_GROUPS.steepEmbedded[0],
        pieceClass: 'steepEmbedded',
        placementKind: 'steepEmbedded',
        embedding,
        cliffPlacement,
        cliffMask,
        grade,
        yaw: rollSeed * Math.PI * 2,
        desiredLongAxis,
...
function buildDesertGroundRockPlacements(heightAt = terrainHeightAt) {
    ...
    for (let attempt = 0;
        ...
        const group = groupRoll < 0.23
            ? 'largeGroundSlab'
            : groupRoll < 0.55 ? 'mediumGroundIrregular' : 'smallRounded';
...
function buildDesertEmbeddedRockPlacements(heightAt, occupiedPlacements) {
    const placements = [];
    const allPlacements = occupiedPlacements.slice();
    const naturalMaximumAttempts = DESERT_ROCK_EMBEDDED_NATURAL_TARGET * 1536;
    for (let attempt = 0;
        attempt < naturalMaximumAttempts
            && placements.length < DESERT_ROCK_EMBEDDED_NATURAL_TARGET;
        attempt++
    ) {
        const nearField = hash2(attempt, 103, 877) < 0.70;
        const halfExtent = nearField ? HALF_NEAR - 24 : 720;
        const x = (hash2(attempt, 107, 881) * 2 - 1) * halfExtent;
        const z = (hash2(attempt, 109, 883) * 2 - 1) * halfExtent;
        if (!nearField && Math.max(Math.abs(x), Math.abs(z)) < HALF_NEAR + 32) continue;
        if (desertRockCommonExclusionAt(x, z)) continue;
        if (authoredCliffMaskAt(x, z) > 0.04) continue;
        const { grade } = desertRockGradeAt(heightAt, x, z);
        if (grade < DESERT_ROCK_NATURAL_EMBEDDED_MIN_GRADE || grade > 1.35) continue;
        const candidate = makeEmbeddedRockPlacement({
            x,
            z,
            grade,
            nearField,
            rollSeed: hash2(attempt, 113, 887),
            sizeSeed: hash2(attempt, 127, 907),
            burySeed: hash2(attempt, 131, 911),
            embedding: 'naturalSlope',
        });
        if (!desertRockHasClearance(
            allPlacements,
            x,
            z,
            desertRockSpacingDiameter(candidate),
            nearField ? 6.5 : 9.5,
        )) continue;
        placements.push(candidate);
        allPlacements.push(candidate);
    }
    if (placements.length !== DESERT_ROCK_EMBEDDED_NATURAL_TARGET) {
        throw new Error(
            `Desert embedded rock scatter produced ${placements.length} of `
            + `${DESERT_ROCK_EMBEDDED_NATURAL_TARGET} requested natural placements`,
        );
    }

    // Search each authored shoulder lazily in round-robin passes. This keeps
    // coverage balanced without forcing four rocks onto a shoulder whose
    // rendered horizon triangles only contain two suitable locations, and it
    // stops evaluating the expensive height field as soon as the budget fills.
    const maximumAttempts = DESERT_ROCK_EMBEDDED_CLIFF_TARGET * 256;
    const candidateCursors = DESERT_CLIFF_PLACEMENTS.map(() => 0);
    let acceptedCliffPlacements = 0;
    while (acceptedCliffPlacements < DESERT_ROCK_EMBEDDED_CLIFF_TARGET) {
        let roundMadeProgress = false;
        for (let cliffIndex = 0;
            cliffIndex < DESERT_CLIFF_PLACEMENTS.length
                && acceptedCliffPlacements < DESERT_ROCK_EMBEDDED_CLIFF_TARGET;
            cliffIndex++
        ) {
            const cliff = DESERT_CLIFF_PLACEMENTS[cliffIndex];
            while (candidateCursors[cliffIndex] < maximumAttempts) {
                const attempt = candidateCursors[cliffIndex]++;
                const keyedAttempt = cliffIndex * maximumAttempts + attempt;
                const localX = (hash2(keyedAttempt, 137, 919) * 2 - 1)
                    * (cliff.halfLength + 20) * 1.08;
                const localZ = (hash2(keyedAttempt, 139, 929) * 2 - 1)
                    * (cliff.halfDepth + 24) * 1.08;
                ...
                if (cliffMask < DESERT_ROCK_CLIFF_SHOULDER_MASK_RANGE[0]
                    || cliffMask > DESERT_ROCK_CLIFF_SHOULDER_MASK_RANGE[1]
                    || authoredCliffPlacementMaskAt(cliff, x, z)
                        < DESERT_ROCK_CLIFF_SHOULDER_MASK_RANGE[0]
                ) continue;
                if (desertRockCommonExclusionAt(x, z)) continue;
                const { grade } = desertRockGradeAt(heightAt, x, z);
                if (grade < DESERT_ROCK_CLIFF_EMBEDDED_MIN_GRADE || grade > 1.35) continue;
                const candidate = makeEmbeddedRockPlacement({
                    x,
                    z,
                    grade,
                    nearField: Math.max(Math.abs(x), Math.abs(z)) <= HALF_NEAR,
                    rollSeed: hash2(keyedAttempt, 149, 937),
                    sizeSeed: hash2(keyedAttempt, 151, 941),
                    burySeed: hash2(keyedAttempt, 157, 947),
                    embedding: 'cliffShoulder',
                    cliffMask,
        ...
        if (!roundMadeProgress) {
            throw new Error(
                `Desert embedded rock scatter produced ${acceptedCliffPlacements} of `
                + `${DESERT_ROCK_EMBEDDED_CLIFF_TARGET} requested cliff placements`,
            );
        }
    }
    return placements;
...
function buildDesertRockPlacements(heightAt = terrainHeightAt) {
    ...
    const embedded = buildDesertEmbeddedRockPlacements(heightAt, ground);
    const placements = ground.concat(embedded);
    if (placements.length !== DESERT_ROCK_INSTANCE_TARGET) {
        throw new Error(
            `Desert rock scatter produced ${placements.length} of `
            + `${DESERT_ROCK_INSTANCE_TARGET} requested total placements`,
        );
    }
    return placements;
}
...
const GROUND_CELLULAR_POLICY = Object.freeze({
    baseCellMeters: 6,
    clusterCells: 2,
    share: 0.74,
    jitter: 0.22,
    ...
    warpMeters: 1.32,
    seed: 1229,
    elevationTrends: Object.freeze([-0.18, 0.10, -0.14, -0.10, 0.18, 0.22]),
});

function groundCellHash(x, z, seed) {
    const value = Math.sin(x * 127.1 + z * 311.7 + seed * 74.7) * 43758.5453123;
    return value - Math.floor(value);
}

function groundCellularWeightsAt(x, z, selectors) {
    const policy = GROUND_CELLULAR_POLICY;
    const warpedX = x + (clamp01(selectors[0]) * 2 - 1) * policy.warpMeters;
    const warpedZ = z + (clamp01(selectors[5]) * 2 - 1) * policy.warpMeters;
    const px = warpedX / policy.baseCellMeters;
    const pz = warpedZ / policy.baseCellMeters;
    ...
    for (let dz = -1; dz <= 1; dz++) {
        for (let dx = -1; dx <= 1; dx++) {
            ...
            const categoryX = Math.floor(cellX / policy.clusterCells);
            const categoryZ = Math.floor(cellZ / policy.clusterCells);
            const superX = Math.floor(categoryX / 6);
            const superZ = Math.floor(categoryZ / 6);
            const localX = mod(categoryX, 6), localZ = mod(categoryZ, 6);
            const rotation = Math.floor(groundCellHash(
                superX, superZ, policy.seed,
            ) * 6);
            const orientation = Math.floor(groundCellHash(
                superX, superZ, policy.seed + 53,
            ) * 4);
            const phase = [
                localX + localZ * 3,
                -localX + localZ * 3,
            ...
            const category = mod(phase + rotation, 6);
            const jitterX = (groundCellHash(
                cellX, cellZ, policy.seed + 17,
            ) * 2 - 1) * policy.jitter;
            const jitterZ = (groundCellHash(
                cellX, cellZ, policy.seed + 31,
            ) * 2 - 1) * policy.jitter;
            const offsetX = px - (cellX + 0.5 + jitterX);
            const offsetZ = pz - (cellZ + 0.5 + jitterZ);
            scores[category] += Math.exp(
                -policy.sharpness * (offsetX * offsetX + offsetZ * offsetZ),
            );
        }
    }
    const total = scores.reduce((sum, score) => sum + score, 0);
    const normalized = scores.map((score) => score / Math.max(total, 1e-8));
    // WebGPU exposes normalized 8-bit attributes in x2/x4 formats, not a
    // portable normalized scalar. Two RGBA8 attributes are therefore the
    // smallest safe layout for six independently interpolated weights.
    const packed = normalized.map((weight) => Math.round(clamp01(weight) * 255));
    const decoded = packed.map((weight) => weight / 255);
    const decodedTotal = decoded.reduce((sum, weight) => sum + weight, 0);
    return {
        packed,
        weights: decoded.map((weight) => weight / Math.max(decodedTotal, 1e-8)),
    };
}
...
function resolvePaintOwnership(
    ...
    const features = selectors.map((selector, index) => (
        clamp01(selector * 0.80 + shaped[index] * 0.20)
    ));
    ...
    const rockEligibility = Math.max(heightRock, slopeRock);
    ...
    const sparseFamily = (scores, softness = 0.24) => {
        let firstIndex;
        let secondIndex;
        ...
        const separation = smooth(0, edgeSoftness, margin);
        const weights = scores.map(() => 0);
        weights[firstIndex] = 0.5 + 0.5 * separation;
        weights[secondIndex] = 0.5 - 0.5 * separation;
        return { weights, margin, firstIndex, secondIndex };
    };

    const groundCellular = groundCellularWeightsAt(worldX, worldZ, selectors);
    ...
    const groundScores = groundPrimaries.map((primary, index) => (
        (fieldScore(primary) * (1 - GROUND_CELLULAR_POLICY.share)
            + groundCellular.weights[index] * GROUND_CELLULAR_POLICY.share)
        * (1 + GROUND_CELLULAR_POLICY.elevationTrends[index] * groundHeightSignal)
    ));
    const washScores = [
    ...
    const rockOwnership = sparseFamily(rockScores);
...
function addSurfaceAttributes(T3, geometry) {
    ...
    const groundCellA = new Uint8Array(position.count * 4);
    const groundCellB = new Uint8Array(position.count * 4);
    for (let index = 0; index < position.count; index++) {
        ...
        groundCellA.set(paint.ownership.groundCellular.packed.slice(0, 4), index * 4);
        groundCellB[index * 4] = paint.ownership.groundCellular.packed[4];
        groundCellB[index * 4 + 1] = paint.ownership.groundCellular.packed[5];
    ...
    geometry.setAttribute(
        'terrainGroundCellA', new T3.Uint8BufferAttribute(groundCellA, 4, true),
    );
    geometry.setAttribute(
        'terrainGroundCellB', new T3.Uint8BufferAttribute(groundCellB, 4, true),
    );
}
...
async function loadGroundTextures(T3, renderer = null) {
    ...
    const surfaceLayers = [
        ...
        {
            id: 'RocksGround02', semantic: 'rock-strewn-ground-transition',
...
function makeTerrainMaterial(T3, maps, { far = false } = {}) {
    ...
    const groundCellA = T3.attribute('terrainGroundCellA', 'vec4').clamp(0, 1);
    const groundCellB = T3.attribute('terrainGroundCellB', 'vec4').clamp(0, 1);
    const groundCellRaw = [
        groundCellA.x, groundCellA.y, groundCellA.z, groundCellA.w,
        groundCellB.x, groundCellB.y,
    ];
    const groundCellSum = groundCellRaw.reduce(
        (sum, weight) => sum.add(weight), T3.float(0),
    ).max(0.00001);
    const groundCellWeights = groundCellRaw.map((weight) => weight.div(groundCellSum));
    const rawSelectors = [
    ...
    const brushSamples = TERRAIN_BLEND_BRUSH_TRANSFORMS.map((transform, index) => {
        ...
        return T3.texture(maps.blendBrush, brushUv).r
    ...
    const features = rawSelectors.map((selector, index) => (
        selector.mul(0.80).add(shapedSelectors[index].mul(0.20)).clamp(0, 1)
    ...
    const rockEligibility = T3.max(heightRock, slopeRock);
    ...
    const sparseFamily = (scores, softness = 0.24) => {
        ...
        const separation = T3.smoothstep(0, edgeSoftness, margin);
        const firstWeight = separation.mul(0.5).add(0.5);
        const secondWeight = T3.float(0.5).sub(separation.mul(0.5));
        const weights = scores.map((unused, index) => (
            firstIndex.equal(T3.int(index)).select(
                firstWeight,
                secondIndex.equal(T3.int(index)).select(
                    secondWeight, T3.float(0),
                ),
            )
        ));
    ...
    const groundPrimaries = [
        features[1], features[6], features[2], features[3],
        features[1].oneMinus(), features[3].oneMinus(),
    ];
    const groundElevationTrends = [-0.18, 0.10, -0.14, -0.10, 0.18, 0.22];
    const groundHeightSignal = coarseGround.mul(2).sub(1);
    const groundScores = groundPrimaries.map((primary, index) => (
        fieldScore(primary).mul(0.26)
            .add(groundCellWeights[index].mul(0.74))
    ...
    const rockScores = [
        ...
        fieldScore(features[2]).mul(nonFace),
    ...
    const rawSplatWeights = [
        ...
        groundMembers[5].mul(groundFamily),
    ];
    ...
    const worldGradX = world.dFdx();
    const worldGradY = world.dFdy();
    const selectedSlots = selected.map((entry, slot) => {
        const transform = dynamicTransformFor(entry.layer, slot);
        const uvNode = transformCoordinate(world, transform)
            .toVar(`terrainTopUv${slot}`);
        const gradX = transformCoordinate(worldGradX, transform)
            .toVar(`terrainTopGradX${slot}`);
        const gradY = transformCoordinate(worldGradY, transform)
    ...
    const viewDistance = T3.positionView.z.negate().max(0);
    ...
    // Never feed world-XZ samples through normalMap(), whose implicit TBN uses
    // mesh UV derivatives. Near PlaneGeometry has V opposite world +Z and the
    // horizon has no UV at all. Build one explicit world-XZ frame around the
    // resolved geometric world normal, then transform the result to view space.
    const geometricWorldNormal = T3.normalize(T3.normalWorld);
    const tangentWorld = T3.normalize(
        T3.vec3(1, 0, 0)
            .sub(geometricWorldNormal.mul(geometricWorldNormal.x))
    ...
    material.userData.familyLayers = {
        ground: [0, 1, 2, 3, 10, 13],
        wash: [4, 7, 9],
        rock: [5, 6, 8, 11, 12],
    };
    material.userData.paintPolicy =
        'soft-height-biased_6-ground_3-exact-wash_5-rock_balanced-cellular-filled-ownership';
    material.userData.splatAttributes = [
        'terrainSplatA', 'terrainSplatB', 'terrainGroundCellA', 'terrainGroundCellB',
    ];
    ...
    material.userData.groundCellularAttributeSemantics = [
        'RGBA8_UNORM_ground-children-0-through-3',
        'RG8-of-RGBA8_UNORM_ground-children-4-through-5_BA-padding',
    ];
    material.userData.groundCellularPolicy = {
        baseCellMeters: GROUND_CELLULAR_POLICY.baseCellMeters,
        clusterCells: GROUND_CELLULAR_POLICY.clusterCells,
        effectiveClusterMeters: GROUND_CELLULAR_POLICY.baseCellMeters
            * GROUND_CELLULAR_POLICY.clusterCells,
        share: GROUND_CELLULAR_POLICY.share,
        jitter: GROUND_CELLULAR_POLICY.jitter,
        sharpness: GROUND_CELLULAR_POLICY.sharpness,
        warpMeters: GROUND_CELLULAR_POLICY.warpMeters,
        elevationTrends: [...GROUND_CELLULAR_POLICY.elevationTrends],
        evaluation: 'cpu-once-per-terrain-vertex_zero-fragment-hash-search',
        packing: 'two-RGBA8_UNORM_8-bytes-per-vertex',
    };
    material.userData.transitionPolicy =
        'selector-warped-balanced-cellular_seedthree-brush_soft-irregular-crossfades_height-slope-gates_height-alpha';
    material.userData.geologicStratification = {
        softGroundElevationBiasMeters: [4, 18],
        groundElevationTrends: [...GROUND_CELLULAR_POLICY.elevationTrends],
        ...
        familyMaximumActiveChildren: 2,
        childCrossfadeMargin: 0.24,
    ...
    material.userData.normalBasis =
        'explicit-world-XZ_frame_no-mesh-UV-dependency_top-left-image-normalY-inverted-once';
    ...
    material.userData.topKRetainedMass = {
        samples: 37249,
        minimum: 0.9117724132557293,
        p01: 0.9793761314468411,
        median: 1,
        mean: 0.9985690806705616,
    };
...
export async function makeTerrain(T3, renderer = null) {
    ...
    if (rockCapacities.some((capacity) => capacity === 0)) {
        throw new Error('Deterministic desert rock scatter omitted a source piece');
    }
    ...
    const rockInstanceStates = rockPlacements.map((placement, index) => {
        ...
        const isEmbedded = placement.placementKind === 'steepEmbedded';
        const uniformScale = isEmbedded
            ? placement.desiredLongAxis / Math.max(dimensions.height, 1e-5)
            : placement.desiredDiameter / Math.max(footprint, 1e-5);
        const placementSize = isEmbedded
        ...
        if (isEmbedded) {
            ...
            const longAxis = upslope.multiplyScalar(0.97)
                ...
                .normalize();
            const alignLongAxis = new T3.Quaternion().setFromUnitVectors(rockUp, longAxis);
            const roll = new T3.Quaternion().setFromAxisAngle(rockUp, placement.yaw);
            rotation = alignLongAxis.multiply(roll);
            const transverseThickness = Math.min(dimensions.widthX, dimensions.widthZ)
                * uniformScale;
    ...
    terrain.userData.surfacePaintPolicy =
        'balanced-cellular-6-ground_3-exact-wash_5-elevated-rock_dense-interleaving';
    terrain.userData.surfaceTransitionPolicy =
        'packed-cellular-vertex-ownership_seedthree-authored-brush-per-pixel-independent-thresholds_height-alpha';
    terrain.userData.groundCellularAttributeFormat = '2xRGBA8_UNORM';
    terrain.userData.groundCellularAttributeBytesPerVertex = 8;
    terrain.userData.groundCellularNearAttributeBytes = position.count * 8;
    terrain.userData.groundCellularHorizonAttributeBytes =
        horizonGeometry.getAttribute('position').count * 8;
    terrain.userData.groundCellularTotalAttributeBytes =
        terrain.userData.groundCellularNearAttributeBytes
        + terrain.userData.groundCellularHorizonAttributeBytes;
    terrain.userData.groundCellularFragmentHashSearches = 0;
    ...
    terrain.userData.desertRockChunks = {
        profile: 'user-12-piece-instanced-terrain-dressing-v3',
        ...
        maximumDiameter: Math.max(...rockPlacements
            .filter((item) => item.placementKind === 'ground')
            .map((item) => item.desiredDiameter)),
        embeddedLongAxisRangesMetres: {
            steepEmbedded: DESERT_ROCK_DIAMETER_RANGES.steepEmbedded.slice(),
        },
        groundInstances: rockPlacements
            .filter((item) => item.placementKind === 'ground').length,
        embeddedNaturalInstances: rockPlacements
            .filter((item) => item.embedding === 'naturalSlope').length,
        embeddedCliffShoulderInstances: rockPlacements
            .filter((item) => item.embedding === 'cliffShoulder').length,
        ...
        embeddedMinimumGrades: {
            naturalSlope: DESERT_ROCK_NATURAL_EMBEDDED_MIN_GRADE,
            cliffShoulder: DESERT_ROCK_CLIFF_EMBEDDED_MIN_GRADE,
        },
        cliffShoulderMaskRange: DESERT_ROCK_CLIFF_SHOULDER_MASK_RANGE.slice(),
        embeddedBurialFractionRange:
            DESERT_ROCK_EMBEDDED_BURY_FRACTION_RANGE.slice(),
        deterministicPlacement: true,
        placementVersion: 'desert-rock-authored-v3',
        proceduralRockInstancesRetired: 20220,
        uniformScaleOnly: true,
        bottomCenteredReusableSubmeshes: true,
        terrainAwareSeating: true,
        terrainNormalAlignment: 'ground roles only',
        steepLongAxisAlignment: 'upslope tangent with inward lean',
        steepBurialBasis: 'authored transverse thickness',
        exclusions: {
            templeCompound: true,
