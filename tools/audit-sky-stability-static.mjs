...
ok(/const sampleP = pWorld\.add\(u\.sunDir\.mul\(along\)\)/.test(sky)
...
ok(/smoothstep\(0\.02, 0\.16, u\.sunDir\.y\)/.test(sky), 'cloud shadows naturally fade when the sun is below the horizon');
...
ok(/const baseColor4 = T3\.vec4\(original \?\? T3\.materialColor\)[\s\S]*T3\.vec4\(baseColor4\.rgb\.mul\(shade\), baseColor4\.a\)/.test(sky), 'cloud shadows preserve alpha-tested receiver silhouettes');
...
ok(!/suppressedEnvironmentRoots|zeroOpaqueEnvironment/.test(pipeline), 'reflection pipeline preserves native per-material environment shading');
ok(/reflectionCompose: 'native-cloud-pmrem-ibl_then-three-ssr'/.test(pipeline),
    'native cloud PMREM/IBL composes before Three SSR');
ok(!/cloud_reflection_filter|makeCloudReflectionFilter/.test(pipeline),
    'no receiver-pixel cloud blur can reintroduce oil-band colours');
