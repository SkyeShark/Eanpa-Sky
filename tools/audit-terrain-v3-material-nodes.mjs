...
for (const far of [false, true]) {
    ...
    assert(material.userData.normalBasis.startsWith('height-sheared-XZ_explicit-frame')
        && material.userData.surfaceProjection
            === 'continuous-world-height-sheared-XZ_with-slope-organic-warp_single-sample-per-layer'
        && JSON.stringify(material.userData.surfaceProjectionHeightShear) === '[0.61,-0.47]'
        && JSON.stringify(material.userData.slopeProjectionGradeSignal) === '[0.12,0.3]'
        && material.userData.slopeProjectionWarpMeters === 1.10,
    'Terrain declares a continuous height-sheared slope projection and matching explicit normal basis');
    ...
    assert(explicitGradientSamples.length === 10,
        'Generated WGSL contains exactly five albedo and five packed dynamic samples');
    ...
    assert(/selectedSlots\[index\]\.transform\.y/.test(source)
        && /selectedSlots\[index\]\.transform\.z/.test(source)
