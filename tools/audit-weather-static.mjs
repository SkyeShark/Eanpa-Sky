...
ok(/const N_STORM_STEPS = Math\.max\(4, Math\.round\(Number\(\s*globalThis\.Deno\?\.env\?\.get\?\.\('STORMSTEPS'\) \?\? opts\.stormSamples \?\? 12\)\)\)/.test(skySource)
...
ok(/stormShadowExtinctionAt\(sampleP\)/.test(skySource)
    ...
    && /daylight = smoothstep\(0\.02, 0\.16, u\.sunDir\.y\)[\s\S]*u\.celestialVisibility/.test(skySource),
...
for (const key of ['rainK', 'wetness', 'denseA', 'streakLen', 'fallMul', 'dashK', 'cellLo', 'cellHi', 'windVec']) {
...
ok(/opacityBaseRange: \[0\.075, 0\.20\]/.test(weatherSource), 'runtime diagnostics expose the restrained transparent-drop alpha range');
...
ok(/const upMask = clamp\(normalWorld\.y, 0, 1\)\.pow\(2\)\.mul\(u\.wetness\)/.test(weatherSource), 'wetness uses per-fragment world normal');
...
ok(/const puddleGate = \(mat\.userData\?\.noPuddles \|\| receiver\?\.userData\?\.noPuddles\)/.test(weatherSource), 'receivers with authored foliage normals can reject ground-only puddles');
...
ok(/const ringWispFlatY = u\.cloudStart\.add\(u\.cloudHeight\)\.add\(1000\);[\s\S]*const ringWispYAt = \(zq\) => ringDeckY\(zq\)\.sub\(RING_BASE\)\.add\(ringWispFlatY\);[\s\S]*const ringWispTLo = ringWispFlatY\.sub\(org\.y\)\.div\(ringWispDy\)\.toVar\(\);[\s\S]*const ringWispTHi = ringWispFlatY\.add\(RING_RISE\)\.sub\(org\.y\)\.div\(ringWispDy\)\.toVar\(\);[\s\S]*solveStep < 8[\s\S]*ringWispTLo\.assign\(ringWispTMid\)[\s\S]*ringWispTHi\.assign\(ringWispTMid\)[\s\S]*const wispHitT = RING_R \? ringWispT : shellFar/.test(skySource),
...
ok(/const ringWispProgress = smoothstep\([\s\S]*float\(RING_ZFLAT\), float\(RING_VISIBLE_END\), pC\.z\.abs\(\)[\s\S]*const ringWispHalf = mix\(uLocalHalf, uWispCurveHalf, ringWispProgress\);/.test(skySource),
...
ok(/const ringWispWidthMask =[\s\S]*const ringWispEndMask =[\s\S]*const wispW = RING_R \? ringWispWidthMask\.mul\(ringWispEndMask\) : float\(1\)[\s\S]*const wispD = canopyShape\.mul\(u\.wispOn\)\.mul\(u\.wispStrength\)\.mul\(wispW\)/.test(skySource), 'Ringworld high clouds retain curved-deck width/end containment');
...
const highLayerSource = skySource.slice(
    skySource.indexOf('const ringWispFlatY ='),
    skySource.indexOf('// distance fade into horizon haze'),
);
...
ok(/const canopyField = smoothstep\([\s\S]*wispPatch\.mul\(0\.72\)\.add\(wispBend\.add\(0\.44\)\.mul\(0\.28\)\)[\s\S]*const canopyDetail = smoothstep\(0\.20, 0\.82, wispN\)\.mul\(0\.45\)\.add\(0\.55\);[\s\S]*const canopyShape = max\(wispShape, canopyField\.mul\(u\.wispFloor\)\.mul\(canopyDetail\)\);[\s\S]*const wispD = canopyShape/.test(highLayerSource),
...
ok(/import\('\.\/cloudspatial\.js\?ts=' \+ Date\.now\(\)\)/.test(sources['src/main.js']), 'active app imports the current-frame spatial compositor');
...
ok(/const wispHitT = RING_R \? ringWispT : shellFar/.test(skySource), 'Ringworld high clouds use the shared quality-independent curved field');
...
ok(/bandNormal: await load\('\.\/assets\/ringworld\/ring_band_normal_v2\.png', \{\}\)/.test(ringWrapperSource), 'Ringworld band normal loads mip-free for exact eidoverse SPOM parity');
...
ok(/bandHeight: await load\('\.\/assets\/ringworld\/ring_band_height\.png', \{\}\)/.test(ringWrapperSource)
...
ok(/const Tn = T3\.normalize\(vec3\(0, posL\.z\.negate\(\), posL\.y\)\);[\s\S]*const Bn = vec3\(1, 0, 0\);[\s\S]*const Nn = _envRG\('RINGNGEO'\) === '0'\s*\? T3\.normalLocal\s*: T3\.normalize\(vec3\(float\(0\), posL\.y, posL\.z\)\)\.negate\(\);/.test(ringEngineSource),
    'water normals use the analytic cylindrical tangent frame (geometric inward normal, RINGNGEO=0 comparison escape)');
